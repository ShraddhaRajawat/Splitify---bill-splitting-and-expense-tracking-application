...
export const getUserBalances = query({
  handler: async (ctx) => {
    ...
    const user = await ctx.runQuery(internal.users.getCurrentUser);  // -- check wheather user is authenticated by a centralized logic

    /* ───────────── 1‑to‑1 expenses (no groupId) ───────────── */
    // filter expenses to only include one-on-one expenses (not group expenses)
    // where the current user is either payer or int he splits
    ...
    for (const e of expenses) {
      ...
      } else if (mySplit && !mySplit.paid) { // someone else paid and user hasn't paid their split
    ...
    // get settlements that directly invlove the current user
